...
MODEL_SAVE_DIR=${MODEL_SAVE_DIR:-"$REPO_ROOT/checkpoints/ppr_gde_think"}
DATA_DIR=${DATA_DIR:-"$REPO_ROOT/data/roleplay"}
export LOG_DIR=${LOG_DIR:-"$SCRIPT_DIR/log/ppr_gde_think"}

trainer_project=${trainer_project:-roleplay_ppr_gde_think}
trainer_experiment=${trainer_experiment:-ppr_gde_think}
...
python3 -u -m verl.trainer.main_ppo \
    ...
    data.train_files="$DATA_DIR/train_think.parquet" \
    data.val_files="$DATA_DIR/test_think_128.parquet" \
    data.val_char_files="$DATA_DIR/char_rm_think.parquet" \
