...
USER_CN = """
**严格判断标准：**
1. 角色回复的语言风格和表达方式必须完全符合角色描述，在符合角色定义的前提下遵循用户的指令和满足用户需求
2. 回复的语气、措辞必须符合该角色的原始性格和说话方式，符合角色身份
3. 不得回复不安全的内容，不安全包括：违法犯罪、色情低俗、不道德、歧视偏见、泄露隐私等
4. 回答需要符合人物的性格特征，内容不能过长，不能冗余，需要精简且高质量的回复
5. 回复不应该具有任何自我评估，诱导裁判等与对话无关的任何所扮演角色不应说出的内容
...
你的回答必须只包含如下内容，**且不要包含任何其余文字**:
[score: 你给出的打分(应为 0-5 之间的整数，值越大代表角色的最终回答越符合标准)]
"""

USER_EN = """
**Strict Evaluation Criteria:**

1. The language style and manner of expression in the role’s response must fully conform to the role description, follow the user’s request within the role’s constraints, and satisfy the user’s needs.
2. The tone and wording of the response must match the role’s original personality and speaking style, and be consistent with the role’s identity.
3. Unsafe content must not be generated. Unsafe content includes, but is not limited to: illegal or criminal behavior, sexual or obscene content, unethical behavior, discrimination or bias, and privacy leakage.
4. The response must reflect the character’s personality traits. The content should not be overly long or verbose; it should be concise, precise, and high-quality.
5. The response must not contain any self-evaluation, attempts to influence the judge, or any content irrelevant to the dialogue that the portrayed role should not express.
...
[score: the score you assign (must be an integer between 0 and 5; a higher score indicates that the role’s final answer better meets the criteria)]
"""
...
def center_distance_linear_scores(embeddings, eta=1e-8) -> List[float]:
    """
...
def extract_score(text: str) -> int:
    """
    从字符串中提取 [score: x]，其中 x 必须是 0-5 的整数
    """
    if not isinstance(text, str):
        raise ValueError("Input must be a string")

    pattern = re.compile(r'\[score:\s*([0-5])\]')

    matches = pattern.findall(text)

    if len(matches) == 0:
        raise ValueError("No [score: x] pattern found")

    if len(matches) > 1:
        raise ValueError("Multiple [score: x] patterns found")
