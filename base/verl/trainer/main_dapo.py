...
"""
from verl.trainer.ppo.dapo_ray_trainer import RayDAPOTrainer
from verl.utils.reward_score import gsm8k, math_verify, table, MATH
...
def compute_score(data_source, solution_str, ground_truth, prompt_str, extra_info=None):
    ...
    if "code executor" in prompt_str:
    ...
    res, rethink_sample = compute_score_fn(solution_str, ground_truth, prompt_str)

    print(f"  Total Score: {res}")
    print("=" * 80 + "\n")

    if isinstance(res, (int, float, bool)):
        return float(res), rethink_sample
    else:
        return float(res[0]), rethink_sample
