...
def compute_reward_metrics(batch):
    ...
    reward_metrics = {}
    reward_metrics["reward/mean"] = torch.mean(reward_tensor).detach().item()   # 所有样本的奖励均值
    ...
    thinking_ratio = torch.sum(batch.batch['thinking_scores'] > 0).float() / reward_tensor.numel()
    reward_metrics["reward/thinking"] = thinking_ratio.detach().item()
...
class RayPPOTrainer(object):
    ...
    def _gen_CharacterEval(self, repeat_times=1):
        ...
        return data_set, sample_inputs, sample_outputs
        
        
    def _val_CharacterEval(self, data_set, sample_inputs, sample_outputs):
        ...
        # ==== 记录生成示例到 WandB ====
        # 将 sample_scores 合并后才能传递到日志
        flat_scores = []
        for lst in sample_scores.values():
            flat_scores.extend(lst)
        ...
        for task_name, score_list in sample_scores.items():
            ...
            wandb_key = f"val/test_score/character/{task_name}"
    ...
    def _val_rolellm(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # 2. 定义两个并行线程：一个评估 RoleLLM，一个跑 Diversity
            t1 = threading.Thread(target=lambda: res_rolellm.setdefault("res", self.role_llm(test_batch, self.global_steps, self.thinking)))
            t2 = threading.Thread(target=lambda: res_diversity.setdefault("res", self.diversity(copy.deepcopy(test_batch), self.global_steps, self.thinking)))
            ...
            grouped_scores = res_rolellm["res"]
            grouped_scores.update(res_diversity["res"])
            
            # 分类收集
            for task_name, arr in grouped_scores.items():
                sample_scores[task_name].extend(arr.tolist())

        print("✅ RoleLLM 验证评分完成!")
        ...
        for task_name, score_list in sample_scores.items():
            ...
            wandb_key = f"val/test_score/{task_name}"
            metric_dict[wandb_key] = float(mean_v)
    ...
    def _validate(self):
        data_set, sample_inputs, sample_outputs = self._gen_CharacterEval()
        ...
        t1 = threading.Thread(target=lambda: res_rolellm.setdefault("res", self._val_rolellm()))  # 包含本地推理 + 评估
        t2 = threading.Thread(target=lambda: res_char_rm.setdefault("res", self._val_CharacterEval(data_set, sample_inputs, sample_outputs)))
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        try:
                            ...
                            use_soft_format = self.global_steps <= 200
                            # thinking 决定是否令模型执行thinking推理, use_soft_format 决定对于thinking格式打分是否严格
                            reward_tensor, batch_new_samples, thinking_tensor = self.reward_fn(batch, self.thinking, use_soft_format)    # 调用vllm服务进行打分
                            if batch_new_samples:
                            ...
                            batch_new_samples = []
                        batch.batch['token_level_scores'] = reward_tensor + thinking_tensor
