...
def compute_reward_metrics(batch):
    ...
    # format_error = torch.sum(-0.5 >= reward_tensor > -1).float() / reward_tensor.numel()
...
class RayDAPOTrainer(RayPPOTrainer):
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('reward', timing_raw):
                        ...
                        try:
                            reward_result, batch_rethink_samples = self.reward_fn(new_batch, return_dict=True)
                            reward_tensor = reward_result['reward_tensor']
                            reward_extra_infos_dict = reward_result['reward_extra_info']
                        except Exception as e:
                            print(f'Error in reward_fn: {e}')
                            reward_tensor, batch_rethink_samples = self.reward_fn(new_batch)
                            reward_extra_infos_dict = {}

                        if batch_rethink_samples:
                            all_rethink_samples += batch_rethink_samples
                        new_batch.batch['token_level_scores'] = reward_tensor

                        # reward
                        reward_metrics = compute_reward_metrics(new_batch)
            ...
            # if all_rethink_samples:
            #     self._update_train_dataloader(all_rethink_samples)
