...
class ThinkRewardManager:
    ...
    def compute_diversity_rewards(self, data: DataProto, group_resonses: list, group_size = 8):
        ...
        for group in group_resonses:
            ...
            if len(responses) != self.config.actor_rollout_ref.rollout.n:
                self.print("合规样本过少, 跳过 diversity 计算")
                continue

            if len(indices) != len(responses):
                self.print("index 与 responses 数量不一致, 跳过 diversity 计算")
                continue

            # ---------- 一次性计算 embeddings ----------
            texts = [res['response'] for res in responses]
            ...
            for start in range(0, total, group_size):
                ...
                sub_responses = responses[start:end]

                # 计算当前子组 diversity reward
                scores = center_distance_linear_scores(sub_embeddings)

                if len(sub_indices) != len(scores):
                    self.print("索引个数与得分数不一致, 跳过该子组 diversity 计算")
                    continue

                for index, score, response in zip(sub_indices, scores, sub_responses):
