...
class ActorRolloutRefWorker(Worker):
    ...
    def update_actor(self, data: DataProto):
        """ 优化模型参数，并将关键信息保存在 output['metrics'] 中  """
        # Support all hardwares
        data = data.to(torch.cuda.current_device())
        assert self._is_actor
        # 如果启用 offload，则从cpu上将参数拉回到gpu        False
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data) # 将输入数据切分到各gpu上
            # perform training
            # TODO:::::::::: 修改关键
            with Timer(name='update_policy', logger=None) as timer:
                metrics = self.actor.update_policy(data=data)           # 通过之前计算的 loss，进行参数优化
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        with self.rollout_sharding_manager:     # 自动分发任务与收集结果

            # after parameters sync with rollout, offload actor model to CPU
            if self._is_offload_param:              # False
                offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            if self._is_offload_optimizer:          # False
                offload_fsdp_optimizer(optimizer=self.actor_optimizer) 

            # 记录显存状态
            log_gpu_memory_usage('After entering rollout sharding manager', logger=logger)

            # 生成输出, prompts 是在数据集中就已经处理过的，自带system prompt
            prompts = self.rollout_sharding_manager.preprocess_data(prompts)
            output = self.rollout.generate_sequences(prompts=prompts)

            log_gpu_memory_usage('After rollout generation', logger=logger)

            # 将各显卡输出结果进行gather, 重组为原本的batch顺序
