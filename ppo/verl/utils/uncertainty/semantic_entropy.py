...
def get_semantic_ids_by_embedding(strings_list, model, distance_threshold = 0.2):
    embeddings = model.encode(
        strings_list,
        batch_size=len(strings_list),
        normalize_embeddings=True,  # 非常重要：方便 cosine 距离
    )
    
    clustering = AgglomerativeClustering(
        n_clusters=None,                    # 关键：不指定类别数
        metric="cosine",
        linkage="average",
