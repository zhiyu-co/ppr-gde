...
def save_wandb_run_record(
    project: str,
    name: str,
    run_id: str,
    record_file: str = "wandb/wandb_runs.jsonl",
):
...
def find_wandb_run_id(
    ...
    with open(record_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if record["project"] == project and record["name"] == name:
...
class Tracking(object):
    ...
    def __init__(self, project_name, experiment_name, default_backend: Union[str, List[str]] = 'console', config=None):
        ...
        if 'tracking' in default_backend or 'wandb' in default_backend:
            ...
            run_id = find_wandb_run_id(project_name, experiment_name)
            
            if run_id is not None:
                print("~"*20 + f"Resuming wandb run: {run_id}" + "~"*20)
                wandb.init(
                ...
                print("~"*20 + "Starting new wandb run" + "~"*20)
                wandb.init(project=project_name,
                        name=experiment_name,
                        mode="offline",
                        settings=wandb.Settings(console="off"))      # 禁止输出wandb日志信息
                save_wandb_run_record(project_name, experiment_name, wandb.run.id)

            # os.environ["WANDB_DISABLED"] = "true"         禁用 wandb记录
            self.logger['wandb'] = wandb
