...
from verl.utils.reward_score.diversity import distinct_n_corpus_level, self_bleu


def strip_think_keep_answer(text: str) -> str:
    """
    如果模型输出包含thinking标签，则只保留<answer>...</answer>的内容。
    如果不包含，则返回原文。
    """
    # 匹配 <answer>...</answer>
    ...
    # 如果有think但没有answer，就保留原文以防信息丢失
    return text


RAW_PROMPT = """你是一名知识丰富的模型性能评估专家，你熟悉并了解各种问答任务可能涉及的知识。
现在，你需要协助用户进行模型回复质量的准确性评估，主要考量模型针对问题的回复是否**符合客观事实或应有的表现**。
注意：你应**忽略回复中的风格或个性描述**，而只**聚焦于其最本质的描述内容是否合理，无需纠结具体细节，大意相同即认为合理**。
用户将会给出模型接受到的问题、输出的回答、以及可供参考的正确答案。
...
现在，你需要协助用户进行模型回复质量的准确性评估，主要考量模型针对问题的回复是否**符合角色{role}的语言风格**。
注意：你应**忽略回复中的具体回答内容**，而只**聚焦于其是否遵循了{role}的说话风格以及习惯**，回复越符合角色描述，越有特色得分越高。
...
现在，你需要协助用户进行模型回复质量的准确性评估，主要考量模型针对问题的回复是否**符合角色{role}所具备的知识或记忆**。
注意：你应**聚焦于该回答是否遵循了{role}所了解的知识和经历，不应出现任何角色不了解的事物**，回复涵盖越多与角色相关的知识和记忆，越丰富得分越高。
以下是对{role}的描述，你应当参考它们进行打分：{role_desc}
用户将会给出模型接受到的问题、输出的回答、以及可供参考的正确答案。
**你的输出应为可被json.load解析的json格式!**
输出共包含两项：reason(解释你给出该打分的理由，应为字符串格式) 以及 score(给出你的打分，该分数应为0-5之间的整数，越大越好)"""
...
class RolellmRewardManager:
    ...
    def _build_messages(self, role_name: str, role_desc: str, question: str, model_answer: str, truth_answer, task_id: str):
        ...
        # 2️⃣ 构造正确答案字符串
        if isinstance(truth_answer, (list, tuple)):
            if len(truth_answer) == 1:
                truth_text = truth_answer[0]
            else:
                truth_text = "\n".join(
                    [f"{i+1}. {ans}" for i, ans in enumerate(truth_answer) if ans.strip()]
                )
        elif isinstance(truth_answer, str):
            truth_text = truth_answer
        else:
            truth_text = str(truth_answer)
    ...
    def _judge_model(self, role_name: str, role_desc: str, question: str, model_answer: str, truth_answer: str, task_name: str, max_try: int = 3, has_thinking = False) -> int:
        """调用vLLM裁判，返回1表示模型胜，0表示负/平/解析失败"""
        import os, re, json, time
        ori_answer = model_answer
        if has_thinking:
            model_answer = strip_think_keep_answer(model_answer)
        messages = self._build_messages(role_name, role_desc, question, model_answer, truth_answer, task_name)
    ...
    def __call__(self, data: DataProto, step=0, has_thinking=False, log_path=None):
        ...
        for i in range(len(data)):
            ...
            truth_answer = extra_info.get('truth_answer', [])
            if truth_answer.size == 0:
                print(f"[warn] 样本 {i} 缺少 truth_answer，跳过评分。")
                continue

            task_name = extra_info.get('task_name', '')
            if task_name == '':
                print(f"[warn] 样本 {i} 缺少 task_name，跳过评分。")
                continue
            ...
            # 将第一次 judge_model 的结果写入 log_list
            log_list.append({
                "score": score,
                "task_name": task_name,
        ...
        print("="*20 + f"rolellm use time: {time()-begin_time}")
