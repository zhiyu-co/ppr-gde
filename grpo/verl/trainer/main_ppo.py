...
def main_task(config):
    ...
    from verl.workers.reward_manager import GRPORewardManager, RolellmRewardManager, CharacterRewardManager, DiversityRewardManager
    from config import CHAR_API_BASE, EMBEDDING_API_BASE
    
    # 奖励计算器，调用vllm提供的模型进行分数计算
    naive_log = Logger("naive")
    reward_fn = GRPORewardManager(tokenizer=tokenizer, embedding_url=EMBEDDING_API_BASE, config=config, p_function=naive_log.log)
